...
datnum=13
cutter=10
mode=5 # 0: 3D plot, 1: 2d plot 2: zeta plot 3: animation 4: zeta - s plot 5: time - zeta plot 6: time - flux plot
...
def title_format(idx):
    return f'{(4.5+0.5*idx):.2f}e4, 2.0e5'
file_path = os.path.join(os.path.dirname(__file__), 'wout_iota0_42.nc')

# dat.dat 파일의 헤더(8행) 건너뛰고 데이터 읽기
...
# zeta 뽑기
    plt.plot(xline,zeta2, 'b', lw=1, label='GC Trajectory', color='tab:blue')
