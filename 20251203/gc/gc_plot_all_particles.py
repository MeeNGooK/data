...
datnum=50   # make sure dat file name is formatted as 'dat1.dat', 'dat2.dat', ...
cutter=5
mode=1
...
if mode==0:
    ...
    for i, (fname, _) in enumerate(file_infos):
        col = cmap(i % getattr(cmap, "N", 10))
        handles_all.append(plt.Line2D([0], [0], marker='o', color='w', markerfacecolor=col, markersize=6))
    ...
    def update(frame):
        ...
        ax_xy.scatter(X3, Y3, color='lightgray', alpha=0.5, s=1)
        ...
        if xs_all:
            sc3 = ax3d.scatter(xs_all, ys_all, zs_all, c=cs_all, cmap=cmap, s=sizes)
            sc2 = ax_xy.scatter(xs_all, ys_all, c=cs_all, cmap=cmap, s=sizes)
        ...
        fig_anim.suptitle(f'init N={datnum}, current N={current_N}')
        ax3d.set_ylabel('Y')
        ax3d.set_zlabel('Z')
        ...
        ax3d.set_title(f't={5.0e-7*cutter*frame:.5f} Frame {frame} (row {frame*cutter})')
