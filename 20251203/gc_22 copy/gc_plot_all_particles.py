...
datnum=2000   # make sure dat file name is formatted as 'dat1.dat', 'dat2.dat', ...
cutter=2
mode=1
elev_=30
azim_=120
...
if mode==0:
    ...
    for i in range(1, datnum + 1):
        ...
        # 👇 여기서 완전히 빈 파일은 바로 스킵
        if data.size == 0:
            print(f"Skipping {fname}: no trajectory data (empty file)")
            continue

        # 한 줄만 있는 1D 배열일 수 있으니 2D로 강제
        data = np.atleast_2d(data)

        nrows = data.shape[0]
        if nrows == 0:
            # 이론상 여기 올 일은 거의 없지만 안전빵으로
            print(f"Skipping {fname}: zero rows after reshape")
            continue
    ...
    def update(frame):
        ...
        ax3d.scatter(X3, Y3, Z3, color='lightgray', alpha=0.4, s=2)
        ax_xy.scatter(X3, Y3, color='lightgray', alpha=0.4, s=2)
        ...
        if xs_all:
            sc3 = ax3d.scatter(xs_all, ys_all, zs_all, color='blue', s=4)
            sc2 = ax_xy.scatter(xs_all, ys_all, color='blue', s=4)
        ...
        ax3d.set_ylabel('Y')
        ax3d.set_zlabel('Z')
        ax3d.view_init(elev=elev_, azim=azim_)
    ...
    out_path = os.path.join(dat_folder, f'dat_all_particles_{elev_}_{azim_}.gif')
    ...
    def update(frame):
        ...
        if xs_all:
            sc3 = ax3d.scatter(xs_all, ys_all, zs_all, color='blue', s=2)
    ...
    out_path = os.path.join(dat_folder, f'dat_all_particles_3_{elev_}_{azim_}.gif')
    writer = PillowWriter(fps=5)
    ani.save(out_path, writer=writer)
    plt.close(fig_anim)
    print(f"Saved animation to {out_path}")